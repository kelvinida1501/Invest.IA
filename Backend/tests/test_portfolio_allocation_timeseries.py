...
def test_portfolio_allocation_endpoint(client, user_token, monkeypatch):
    ...
    monkeypatch.setattr("app.routes.portfolio.ensure_history_for_assets", lambda *args, **kwargs: None)
    monkeypatch.setattr("app.routes.portfolio.get_fx_rate", lambda a, b: (1.0, None))

    resp = client.get("/api/portfolio/allocation", headers=headers, params={"mode": "asset"})
...
def test_portfolio_timeseries_endpoint(client, user_token, monkeypatch):
    ...
    resp = client.get("/api/portfolio/timeseries", headers=headers, params={"range": "1m"})
    assert resp.status_code == 200
    body = resp.json()
