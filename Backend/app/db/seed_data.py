...
def run_seed():
    ...
    try:
        ...
        if not user:
            user = User(name="Kelvin", email="user@example.com", password_hash="hash-demo")
        ...
        btc   = get_or_create("BTC-USD", "Bitcoin", "cripto", "USD")

        # holdings
        h1 = db.query(Holding).filter_by(portfolio_id=portfolio.id, asset_id=petr4.id).first()
        if not h1:
            db.add(Holding(portfolio_id=portfolio.id, asset_id=petr4.id, quantity=10, avg_price=35.0))
        h2 = db.query(Holding).filter_by(portfolio_id=portfolio.id, asset_id=vale3.id).first()
        if not h2:
            db.add(Holding(portfolio_id=portfolio.id, asset_id=vale3.id, quantity=5, avg_price=60.0))

        # news
        n1 = db.query(NewsItem).filter_by(url="https://exemplo.com/1").first()
        if not n1:
            db.add(NewsItem(title="Petrobras anuncia resultados", url="https://exemplo.com/1", sentiment="positivo"))
        n2 = db.query(NewsItem).filter_by(url="https://exemplo.com/2").first()
        if not n2:
            db.add(NewsItem(title="Mercado vê volatilidade", url="https://exemplo.com/2", sentiment="negativo"))
